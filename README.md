...
>
> 🌐 **在线体验**: [http://110.41.63.14:8080](http://110.41.63.14:8080)
...
应用已部署在云服务器上，直接访问 [http://110.41.63.14:8080](http://110.41.63.14:8080) 即可使用。

部署方式：
1. `git clone` 到服务器 `/opt/vlogify`
2. `npm install` 安装依赖
3. `pip3 install edge-tts` 安装 TTS
4. 配置 `.env` 文件（Agnes API Key）
5. 用 systemd + nginx 反代运行
